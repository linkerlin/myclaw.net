...
**差距**: MiniClaw 用<50 行代码实现纵深防御，myclaw.net 完全缺失

**实施文件**:
- [ ] `src/MyClaw.Core/Dna/PurposeMap.cs` (新增) - PURPOSE_MAP 自检镜像
- [ ] `src/MyClaw.Core/Dna/TelomereGuard.cs` (新增) - 端粒守卫
- [ ] `src/MyClaw.Core/Evolution/SignalDetector.cs` (修改) - 信号检测表增强
- [ ] `templates/AGENTS.md` (修改) - 添加"AI 的思考逻辑"列
- [ ] `src/MyClaw.Skills/UpdateDNA.cs` (修改) - 集成端粒守卫和 PURPOSE_MAP
...
**预计工时**: 6 小时  
**优先级**: 🔴🔴🔴  
**参考**: `改进方案和计划-v4.md` Phase 1

---

#### 2. 器官退化 (完全缺失)

**差距**: MiniClaw 50 次心跳后自动剔除未使用工具，myclaw.net 无此机制

**实施文件**:
- [ ] `src/MyClaw.Core/Ribosome/RibosomePruner.cs` (新增)
- [ ] `src/MyClaw.Core/Evolution/EvolutionEngine.cs` (修改) - 集成修剪器

**验收标准**:
- [ ] 50 次心跳后检查工具使用频率
- [ ] 从未使用的工具自动剔除 (永生工具除外)
- [ ] 退化日志记录到 HEARTBEAT.md

**预计工时**: 3 小时  
**优先级**: 🔴🟡  
**参考**: `改进方案和计划-v4.md` Phase 2
...
**差距**: MiniClaw 30 分钟无活动时主动扫描代码库提取 TODO，myclaw.net 无此机制
...
- [ ] 30 分钟无活动触发
- [ ] 随机扫描源码文件
- [ ] 提取 TODO/FIXME 写入 HORIZONS.md
- [ ] 2 小时内不重复执行

**预计工时**: 4 小时  
**优先级**: 🟡🟢  
**参考**: `改进方案和计划-v4.md` Phase 3

---

#### 4. 菌丝共生网络 (完全缺失)

**差距**: MiniClaw 跨实例知识共享，myclaw.net 每个实例是信息孤岛

**实施文件**:
- [ ] `src/MyClaw.Core/Mycelium/MyceliumNetwork.cs` (新增)

**验收标准**:
- [ ] 分泌孢子 (TOOLS/NOCICEPTION 更新时)
- [ ] 吸收异体孢子
- [ ] 消耗已吸收孢子
- [ ] 跨实例群体免疫
